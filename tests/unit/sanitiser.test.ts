...
describe('sanitiseMetricsPayload', () => {
  ...
  // @req REQ-4.9-4
  it('repoTargets with invalid projectKey → 400', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
      repoTargets: [{ projectKey: 'lowercase', repoSlug: 'valid-repo' }],
    });
    expect(result.status).toBe(400);
  });

  // @req REQ-4.9-4
  it('repoTargets with invalid repoSlug → 400', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
      repoTargets: [{ projectKey: 'SS', repoSlug: 'UPPERCASE' }],
    });
    expect(result.status).toBe(400);
  });

  // @req REQ-4.9-4
  it('repoTargets with valid entries → passes through', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
      repoTargets: [{ projectKey: 'SS', repoSlug: 'my-repo' }],
    });
    expect(result.status).toBe(200);
  });

  // @req REQ-4.9-5
  it('projectKeys exceeding max 20 → 400', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
      projectKeys: Array.from({ length: 21 }, (_, i) => `P${i}`),
    });
    expect(result.status).toBe(400);
  });

  // @req REQ-4.9-5
  it('projectKeys with invalid format → 400', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
      projectKeys: ['lowercase'],
    });
    expect(result.status).toBe(400);
  });

  // @req REQ-4.9-5
  it('valid projectKeys → passes through', async () => {
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
  ...
  // @req REQ-4.9-4
  it('repoTargets exceeding max 50 → 400', async () => {
    const targets = Array.from({ length: 51 }, (_, i) => ({ projectKey: 'SS', repoSlug: `repo-${i}` }));
    const result = await run({
      developerIds: ['alice'],
      startDate: '2024-01-01',
      endDate: '2024-01-31',
