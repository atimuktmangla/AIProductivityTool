...
import { purgeRunLogs, listRunLogs, triggerSyncForUsers, getSyncStatus } from '../../jobs/metricsSync.js';
...
const mockedSyncStatus = vi.mocked(getSyncStatus);
...
describe('syncRouter', () => {
  ...
  // @req REQ-4.8.7-1
  it('POST /trigger returns 409 when sync is already running', async () => {
    mockedSyncStatus.mockReturnValueOnce({
      running: true, lastRunAt: null, nextRunAt: null, runStartedAt: Date.now(),
      activeUsers: ['alice'], completedUsers: [], failedUsers: [],
      totalSyncUsers: 1, configuredUsers: ['alice'], intervalMinutes: 0, scheduledTime: '',
    });
    const res = await req('post', '/api/dashboard/sync/trigger', { developerIds: ['bob'] });
    expect(res.status).toBe(409);
    expect((res.body as { error?: string }).error).toBe('sync_in_progress');
    expect(mockedTrigger).not.toHaveBeenCalled();
  });
