...
const initialSavedSession = hasUrlParams ? null : readSavedSession();
const initialState = {
    ...
    switch (action.type) {
        ...
        case 'FETCH_SUCCESS': {
            ...
            if (state.selectedUsers.length > 0)
                params.set('devs', state.selectedUsers.join(','));
...
export function useDashboard() {
    ...
    const setSelectedRepoTargets = useCallback((targets) => {
        dispatch({ type: 'SET_REPO_TARGETS', payload: targets });
    }, []);
    // Tier 2: project keys only — clears repo chips
    const setSelectedProjects = useCallback((keys) => {
    ...
    const setCompareStartDate = useCallback((date) => {
        dispatch({ type: 'SET_COMPARE_START_DATE', payload: date });
    }, []);
    const setCompareEndDate = useCallback((date) => {
