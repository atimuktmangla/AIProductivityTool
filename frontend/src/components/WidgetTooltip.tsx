 ...
 */
export function WidgetTooltip({ content }: WidgetTooltipProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent | MouseEvent) => {
      if (e instanceof KeyboardEvent && e.key === 'Escape') setVisible(false);
      if (
    ...
    <span className="widget-tooltip-wrap">
      ...
      {visible && (
        <div
          ref={tipRef}
          role="tooltip"
          className={`widget-tooltip-panel${above ? ' widget-tooltip-panel--above' : ''}`}
        >
          {content}
        </div>
      )}
